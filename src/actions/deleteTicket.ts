///////////////////////////////////////
//TICKET DELETION SYSTEM {PATCHED}
///////////////////////////////////////
    ...
    opendiscord.actions.get("opendiscord:delete-ticket").workers.add([
        ...
        new api.ODWorker("opendiscord:delete-channel",1,async (instance,params,source,cancel) => {
            ...
            async function sendTranscript(params) {
                ...
                try {
                    // 1. Create transcript as buffer
                    let transcriptBuffer = await createTranscript(channel, {
                        limit: -1,
                    ...
                    let transcriptHTML = transcriptBuffer.toString("utf-8");
                    ...
                    `;
                    transcriptHTML = transcriptHTML.replace("</head>", `${customCSS}</head>`);

                    // 4. Convert back to buffer
                    transcriptBuffer = Buffer.from(transcriptHTML, "utf-8");

                    // 5. Send transcript file to logs
                    const logChannelId = "transcripts_channel_id";
                    const logChannel = guild.channels.cache.get(logChannelId);

                    if (!logChannel || !(logChannel instanceof TextChannel)) {
                        console.warn("Log channel not found or invalid.");
                        return;
                    }

                    await logChannel.send({
                        content: `📄HTML Transcript for ${channel.name}`,
                        files: [
                            {
            ...
            // ⚡ CALL THE FUNCTION before deleting the channel
            await sendTranscript(params);
