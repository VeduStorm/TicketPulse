 ...
 * It's a utility command which can only be used by the creator of TicketPulse or the owner of the bot.
 ...
 * in case you're helping someone with setting up (or debugging) TicketPulse.
 * 
 * CAN I DISABLE IT??
...
export const loadDumpCommand = () => {
    ...
    opendiscord.client.textCommands.onInteraction("!OPENTICKET:","dump",async (msg) => {
        if (msg.author.id == "1104705926558130207" || opendiscord.permissions.hasPermissions("developer",await opendiscord.permissions.getPermissions(msg.author,msg.channel,null))){
            //user is bot owner OR creator of TicketPulse :)
            ...
            if (msg.channel.type != discord.ChannelType.GroupDM) msg.channel.send({content:"## The `otdebug.txt` dump is available!",files:[
                new discord.AttachmentBuilder(debug)
                    .setName("otdebug.txt")
                    .setDescription("The TicketPulse debug dump!")
            ]})
        }
    })
}