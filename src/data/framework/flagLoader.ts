...
export const loadAllFlags = async () => {
    opendiscord.flags.add(new api.ODFlag("opendiscord:no-migration","No Migration","Disable TicketPulse data migration on update!","--no-migration",["-nm"]))
    ...
    opendiscord.flags.add(new api.ODFlag("opendiscord:no-easter","No Easter Eggs","Disable little TicketPulse easter eggs hidden in the bot!","--no-easter",["-ne"]))
    opendiscord.flags.add(new api.ODFlag("opendiscord:no-plugins","No Plugins","Disable all TicketPulse plugins!","--no-plugins",["-np"]))
