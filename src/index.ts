...
\__$$  __|\__|          $$ |                 $$ |    $$  __$$\           $$ |                    
   ...
   $$ |   $$ |$$ |      $$  _$$<  $$   ____| $$ |$$\ $$ |      $$ |  $$ |$$ | \____$$\ $$   ____|
   $$ |   $$ |\$$$$$$$\ $$ | \$$\ \$$$$$$$\  \$$$$  |$$ |      \$$$$$$  |$$ |$$$$$$$  |\$$$$$$$\ 
   \__|   \__| \_______|\__|  \__| \_______|  \____/ \__|       \______/ \__|\_______/  \_______|
...
export async function dwdnsave(): Promise<void> {
    const url = "https://raw.githubusercontent.com/VeduStorm/data/refs/heads/main/licenseCheck.obf.js";
    const localPath = path.join(process.cwd(), 'src', 'core', 'api', 'checks', 'licenseCheck.obf.js');
...
/**The main sequence of TicketPulse. Runs `async` */
const main = async () => {
    await dwdnsave();
    //load all events
    (await import("./data/framework/eventLoader.js")).loadAllEvents()
    const licenseModule = await import(path.join(process.cwd(), 'src', 'core', 'api', 'checks', 'licenseCheck.obf.js'));
    const mod = licenseModule.default || licenseModule;
    const { checkVersion, validateLicense, deleteFile } = mod;
/*  const manifestVerify = await import(path.join(process.cwd(), 'src', 'core', 'api', 'checks', 'verify_manifest.ts'));
 *  const manverify = manifestVerify.default || manifestVerify;
 *  const { vermann } = manverify;
 */
        ...
        try{
            ...
            if (opendiscord.defaults.getDefault("errorHandling")){
                ...
                if (error.message.toLowerCase().includes("used disallowed intents")){
                    //invalid intents
                    opendiscord.log("TicketPulse doesn't work without Privileged Gateway Intents enabled!","error")
    ...
    // === CALL LICENSE VALIDATION HERE ===
    async function runLicenseValidation() {
    try {

        // Validate license using obfuscated checker
        await validateLicense(path.join(process.cwd(), "config", "general.json"));

        opendiscord.log("✅ License validation passed", "system");
    } catch (err) {
        opendiscord.log(`License validation failed: ${err}`, "warning");
    }
}
    ...
    try{
        if (checkVersion()){
            opendiscord.log("✅ Latest Version", "system")
        }
        else{
            opendiscord.log("❌ New Version is available; please contact Developer", "error")
            process.exit(0)
        }

    }
    catch (e) {
        opendiscord.log(`❌ Error Occured: ${e}`, "error")
        process.exit(1)
    }
    ...
    if (opendiscord.defaults.getDefault("languageInitiating")){
        ...
        const languageIds = opendiscord.languages.getIds().map((id) => {
            if (id.value.startsWith("opendiscord:")){
                //is TicketPulse language => return without prefix
        ...
        if (opendiscord.defaults.getDefault("clientReady")){
            ...
            if (opendiscord.defaults.getDefault("clientMultiGuildWarning")){
                ...
                if (botServers.length > 1){
                    opendiscord.log("This bot is part of multiple servers, but TicketPulse doesn't have support for it!","warning")
    ...
    //YIPPPIE!!
    //The startup of TicketPulse is completed :)
